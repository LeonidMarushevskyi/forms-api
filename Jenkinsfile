  ...
  try {
   ...
   stage('SonarQube analysis'){
	...
	stage('Deploy Application'){
	   ...
	   sh 'ansible-playbook -e NEW_RELIC_AGENT=$USE_NEWRELIC  -e FORMS_API_VERSION=$APP_VERSION -i $inventory deploy-forms-api.yml --vault-password-file ~/.ssh/vault.txt -vv'
	   cleanWs()
	   sleep (20)
  }
  stage('Smoke Tests') {
